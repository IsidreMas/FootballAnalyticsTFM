...
def plot_pitch(figax = None, field_dimen = (106.0,68.0), field_color ='green', linewidth=2, markersize=20):
    """ plot_pitch
    ...
    """
    # ALL DIMENSIONS IN m
    border_dimen = (4,4) # include a border arround of the field of width 3m
    ...
    goal_line_depth = 2*meters_per_yard
    ...
    if(figax == None):
            fig,ax = plt.subplots(figsize=(6,4)) # create a figure 
    else:
        fig,ax = figax
    # decide what color we want the field to be. Default is green, but can also choose white
    if  'green' in field_color.split('_'):
        ax.set_facecolor('mediumseagreen')
        lc = 'whitesmoke' # line color
        pc = 'w' # 'spot' colors

        if len(field_color.split('_'))>1:
            number_of_strips = 10
            step = half_pitch_length*2/number_of_strips
            ax.transData.transform([(-half_pitch_length,0), (half_pitch_length,0)])
            pitch_length_pixels = ax.transData.transform((half_pitch_length,0))[0]-ax.transData.transform((-half_pitch_length,0))[0]

            for i in range(number_of_strips+1):
                if 'vertical' in field_color.split('_'):
                    ax.plot([-half_pitch_length+i*step,-half_pitch_length+i*step],[half_pitch_width+border_dimen[0],-half_pitch_width-border_dimen[0]],color = "green",linewidth=pitch_length_pixels/(256*number_of_strips), alpha = 0.5)
                if 'horizontal' in field_color.split('_'):
                    ax.plot([-half_pitch_length -border_dimen[0],half_pitch_length+border_dimen[0]],[half_pitch_width -i*step,half_pitch_width -i*step],color = "green",linewidth=pitch_length_pixels/(256*number_of_strips), alpha = 0.5) 
                if 'circular' in field_color.split('_'):
                    ...
                    ax.plot(x,y,color = "green",linewidth=pitch_length_pixels/(256*number_of_strips), alpha = 0.5)
    ...
    for s in signs: # plots each line seperately
        ...
        # goal posts & line
        ax.plot( [s*half_pitch_length,s*(half_pitch_length+goal_line_depth),s*(half_pitch_length+goal_line_depth),
        s*half_pitch_length],[-goal_line_width/2.,-goal_line_width/2.,goal_line_width/2.,goal_line_width/2.],lc,linewidth=linewidth)
        # 6 yard box
...
def plot_frame( hometeam, awayteam, figax=None, team_colors=('r','b'), field_dimen = (106.0,68.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False ):
    ...
    for team,color in zip( [hometeam,awayteam], team_colors) :
        x_columns = [c for c in team.keys() if 'x' in c.split('_') and c!='ball_x'] # column header for player x positions
        y_columns = [c for c in team.keys() if 'y' in c.split('_') and c!='ball_y'] # column header for player y positions
        ax.plot( team[x_columns], team[y_columns], color+'o', alpha=PlayerAlpha ) # plot player positions
        if include_player_velocities:
            vx_columns = [c for c in team.keys() if 'vx' in c.split('_')] # column header for player x positions
            vy_columns = [c for c in team.keys() if 'vy' in c.split('_')] # column header for player y positions
...
def plot_norm_frame( hometeam, awayteam, figax=None, team_colors=('r','b'), field_dimen = (106.0,68.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False ):
    """ plot_frame( hometeam, awayteam )
    ...
    # plot home & away teams in order
    for team,color in zip( [hometeam,awayteam], team_colors) :
        x_columns = [c for c in team.keys() if c[-6:].lower()=='_normx' and c!='ball_x'] # column header for player x positions
        y_columns = [c for c in team.keys() if c[-6:].lower()=='_normy' and c!='ball_y'] # column header for player y positions
        ax.plot( team[x_columns], team[y_columns], color+'o', alpha=PlayerAlpha ) # plot player positions
        if include_player_velocities:
            vx_columns = [c for c in team.keys() if 'vx' in c.split('_')] # column header for player x positions
            vy_columns = [c for c in team.keys() if 'vy' in c.split('_')] # column header for player y positions
            ax.quiver( team[x_columns], team[y_columns], team[vx_columns], team[vy_columns], color=color, scale_units='inches', scale=10.,width=0.0015,headlength=5,headwidth=3,alpha=PlayerAlpha)
        if annotate:
            [ ax.text( team[x]+0.1, team[y]+0.1, x.split('_')[1], fontsize=10, color=color  ) for x,y in zip(x_columns,y_columns) if not ( np.isnan(team[x]) or np.isnan(team[y]) ) ] 
    # plot ball
    #ax.plot( hometeam['ball_x'], hometeam['ball_y'], 'ko', markersize=6, alpha=1.0, linewidth=0)
    return fig,ax
